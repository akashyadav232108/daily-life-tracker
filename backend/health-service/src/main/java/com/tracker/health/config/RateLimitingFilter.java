...
/**
 * Distributed rate-limiting filter for health-service.
 *
 * Specific limits (all configured in application.yml):
 *   - GET /api/health/summary/weekly → summary-queries (20 / 1 min)
...
public class RateLimitingFilter extends OncePerRequestFilter {
    ...
    private BucketConfiguration resolveConfig(String method, String path) {
        if (isWeeklySummary(method, path)) return buildConfig(props.getSummaryQueries());
        return buildConfig(props.getGlobal());
    }

    private String resolveCategory(String method, String path) {
        if (isWeeklySummary(method, path)) return "summary";
        return "global";
    }

    // ── Path matchers ─────────────────────────────────────────────────────────

    private boolean isWeeklySummary(String method, String path) {
        return "GET".equals(method) && path.equals("/api/health/summary/weekly");
    }
